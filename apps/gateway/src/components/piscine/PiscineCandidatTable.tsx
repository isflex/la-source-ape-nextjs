...
interface PiscineCandidatTableProps {
  ...
  formOwner?: string; // Cognito userId of the planning creator (PiscineForm.owner)
...
export default function PiscineCandidatTable({
  ...
  const [timeSlots, setTimeSlots] = useState<
    Record<string, PiscineTimeSlotData>
  >({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showNewCandidatForms, setShowNewCandidatForms] = useState<
    Record<string, boolean>
  >({});
  const [activeDragCandidat, setActiveDragCandidat] = useState<any | null>(
    null,
  );
  ...
  const loadDateSlotsWithCandidats = async () => {
    try {
      ...
      // Public structural reads: explicit apiKey auth mode so guest viewing is
      // deterministic regardless of the client's userPool default auth mode.
      ...
      // Candidat reads are auth-aware: authenticated users fetch full PII via
      // userPool; guests fetch a PII-free selection set via apiKey (counts only).
      const candidatQueryOptions = getCandidatQueryOptions(isAuthenticated);
        ...
        slots
          ...
          .map(async (slot) => {
            const { data: candidats } =
              await client.models.PiscineCandidat.list({
                filter: { piscineDateSlotId: { eq: slot.id } },
                authMode: candidatQueryOptions.authMode,
                ...(candidatQueryOptions.selectionSet
                  ? { selectionSet: candidatQueryOptions.selectionSet as any }
                  : {}),
              });
            ...
            return {
              ...
              // PII fields are absent for guests (count-only selection set) — fall
              // back to empty strings so the shape holds; they are never rendered.
              candidats: ((candidats || []) as Array<Record<string, any>>)
      ...
      const validDateSlots = dateSlotsWithCandidats.filter((slot) => {
        const hasValidTimeSlot =
          slot.piscineTimeSlotId && timeSlotsMap[slot.piscineTimeSlotId];
      ...
      const sortedDateSlots = validDateSlots.sort(
        (a, b) =>
  ...
  }, [piscineFormId, isAuthenticated]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    for (const dateSlot of dateSlots) {
      const candidat = dateSlot.candidats.find(
        (c) => `candidat-${c.id}` === active.id,
      );
      if (candidat) {
  ...
  const onExportCSV = () => {
    try {
      ...
      const totalCandidats = dateSlots.reduce(
        (total, slot) => total + slot.candidats.length,
        0,
      );
      if (totalCandidats === 0) {
      ...
      // CSV column headers
      csvContent +=
        "Date,Jour,Prénom,Nom,Email,Téléphone,Nom de l'enfant,Ordre d'inscription\n";

      // Sort date slots by date and export each candidat
      const sortedDateSlots = [...dateSlots].sort(
        (a, b) =>
          new Date(a.selectedDate).getTime() -
          new Date(b.selectedDate).getTime(),
      );
      ...
      sortedDateSlots.forEach((dateSlot) => {
        ...
        const sortedCandidats = [...dateSlot.candidats].sort(
      ...
      const safeTitle =
        piscineFormData?.title?.replace(/[^a-zA-Z0-9]/g, "_") ||
        "planning_piscine";
      ...
      onMessage?.(
        `Export CSV réussi ! ${totalCandidats} inscription(s) exportée(s).`,
        false,
      );
    } catch (error) {
      ...
      <div style={{ marginTop: "1.5rem" }}>
        ...
        >
          ...
          <div
            className={classNames(
              ...
              flexStyles.isGridGap4,
              flexStyles.isGridCols1,
              flexStyles.isGridCols2MobileMax,
              flexStyles.isAlignItemsCenter,
        ...
        {dateSlots.map((dateSlot) => (
          ...
          >
            ...
            <div
              style={{
                backgroundColor: isOrphanedSlot(dateSlot)
                  ? "#fff3cd"
                  : "#f8f9fa",
                padding: "1rem",
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {formatDate(dateSlot.selectedDate).replace(/^./, (str) =>
                      str.toUpperCase(),
                    )}
                    ...
                    ) : dateSlot.piscineTimeSlotId &&
                      timeSlots[dateSlot.piscineTimeSlotId] ? (
                      <>
                        {" "}
                        • {
                          timeSlots[dateSlot.piscineTimeSlotId].startTime
                        } - {timeSlots[dateSlot.piscineTimeSlotId].endTime}
                      </>
                    ) : null}
                  </Title>
                  <Title level={TitleLevel.LEVEL7}>
                    {dateSlot.candidats.length} inscription
                    {dateSlot.candidats.length > 1 ? "s" : ""}
                  </Title>
                </div>
                ...
                >
                  <Button
                    markup={ButtonMarkup.BUTTON}
                    variant={
                      isAuthenticated && !isOrphanedSlot(dateSlot)
                        ? VariantState.PRIMARY
                        : VariantState.TERTIARY
                    }
            ...
            >
              <div style={{ padding: "1rem" }}>
                ...
                {!isAuthenticated && (
                  ...
                  >
                    <Text style={{ fontSize: "0.875rem", color: "#0369a1" }}>
                      ℹ️ Vous devez être connecté pour vous inscrire à cette
                      session piscine ou pour consulter les détails des
                ...
                {isOrphanedSlot(dateSlot) && dateSlot.candidats.length > 0 && (
                  ...
                  >
                    <Text style={{ fontSize: "0.875rem", color: "#856404" }}>
                      ℹ️ <strong>Date archivée</strong> : Ce créneau horaire
                      n&apos;est plus disponible, mais les inscriptions
                      existantes sont conservées.
                      {isCreatorMode &&
                        " Les participants peuvent être déplacés vers un autre créneau."}
                    </Text>
                  </div>
                )}
                ...
                {dateSlot.candidats.length === 0 ? (
                  ...
                  >
                    <Text style={{ fontStyle: "italic", color: "#666" }}>
                      Aucune inscription pour cette date. Soyez le premier à
                      vous inscrire !
                    </Text>
                  </div>
                ) : !isAuthenticated ? (
                  /* Guests see the count only — personal details are hidden */
                  <div
                    style={{
                      backgroundColor: "#f9f9f9",
                      padding: "1rem",
                  ...
                  >
                    ...
                    >
                      {dateSlot.candidats.length} inscription
                      {dateSlot.candidats.length > 1 ? "s" : ""} confirmée
                      {dateSlot.candidats.length > 1 ? "s" : ""}
                    </Text>
                    <Text style={{ fontStyle: "italic", color: "#666" }}>
                      🔒 Connectez-vous pour voir les inscrits.
                    </Text>
                  </div>
                ) : (
                  <div>
        ...
        >
          <Text style={{ fontWeight: "bold", color: "#0369a1" }}>
            ...
            {dateSlots.reduce(
              ...
              ? "s"
              : ""}{" "}
            au total sur {dateSlots.length} date
            {dateSlots.length > 1 ? "s" : ""} disponible
