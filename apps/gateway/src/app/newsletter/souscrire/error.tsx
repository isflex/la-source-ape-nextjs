...
export default function NewsletterSouscrireError({ error, reset }: ErrorProps) {
  ...
  React.useEffect(() => {
    ...
    logClientError({
      ...
      route: '/newsletter/souscrire/',
      userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : undefined
    })

    // Log to console for debugging
    debug.errorBoundary('[NewsletterSouscrire Error]', {
      name: error.name,
      message: error.message,
      digest: error.digest,
      stack: error.stack,
      isCopilotKitError: error.message?.includes('CopilotKit') || error.message?.includes('useCopilotKit'),
    })
  }, [error])
      ...
      <InfoBlock>
        ...
        <InfoBlockContent>
          <Title level={TitleLevel.LEVEL4}>
            {`Nous sommes désolés, la page Newsletter ne peut pas être chargée.`}
          </Title>
