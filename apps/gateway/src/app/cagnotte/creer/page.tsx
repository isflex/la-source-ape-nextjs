...
import {
  CreerCagnotteList1,
  CreerCagnotteList2,
} from "@src/components/cagnotte/CagnotteInfoLists";
...
export default function CagnotteCreerPage() {
  ...
  useSafeAgentContext({
    description:
      "Current page context - Jackpot/Cagnotte management page for creating and managing fundraising campaigns",
    value: {
      page: "cagnotte/creer",
  ...
  useSafeFrontendTool(
    {
      ...
      description:
        "Publish a DRAFT jackpot so participants can contribute. The jackpot must be in DRAFT status with a future deadline.",
      parameters: z.object({
        jackpotId: z.string().describe("The ID of the jackpot to publish"),
      }),
      handler: async ({ jackpotId }) => {
        const form = forms.find((f) => f.id === jackpotId);
        if (!form)
          return { success: false, error: `Jackpot "${jackpotId}" not found` };

        const status = form.status || "DRAFT";
        if (status !== "DRAFT")
          return {
            success: false,
            error: `Jackpot is not in DRAFT status (current: ${status})`,
          };
        ...
        if (deadline <= new Date())
          return {
            success: false,
            error: "Cannot publish: deadline has passed",
          };
        ...
        try {
          ...
          setCreateSuccess(
            "Cagnotte publiée avec succès ! Les participants peuvent maintenant contribuer.",
          );
          setTimeout(() => setCreateSuccess(null), 5000);
          return {
            success: true,
            message: `Jackpot "${form.title}" published successfully`,
          };
        } catch (err) {
  ...
  useSafeFrontendTool(
    {
      ...
      parameters: z.object({
        jackpotId: z.string().describe("The ID of the jackpot to delete"),
      }),
      handler: async ({ jackpotId }) => {
        const form = forms.find((f) => f.id === jackpotId);
        if (!form)
        ...
        try {
          const response = await client.models.JackpotForm.delete({
            id: jackpotId,
          });
          if (response.errors)
          ...
          return {
            success: true,
            message: `Jackpot "${form.title}" deleted successfully`,
          };
        } catch (err) {
  ...
  useSafeFrontendTool(
    {
      name: "request_payout",
      description:
        "Request a payout for a CLOSED jackpot that has contributions and no pending payout request.",
      parameters: z.object({
        jackpotId: z
          .string()
          .describe("The ID of the jackpot to request payout for"),
      }),
      handler: async ({ jackpotId }) => {
        const form = forms.find((f) => f.id === jackpotId);
        if (!form)
        ...
        if (status !== "CLOSED")
          return {
            success: false,
            error: `Jackpot must be CLOSED to request payout (current: ${status})`,
          };
        if (form.payoutRequested)
          return {
            success: false,
            error: "Payout has already been requested for this jackpot",
          };
  ...
  useSafeFrontendTool(
    {
      ...
      parameters: z.object({
        jackpotSlug: z.string().describe("The slug of the jackpot to view"),
      }),
      handler: async ({ jackpotSlug }) => {
        router.push(`/cagnotte/${jackpotSlug}/`);
        return {
          success: true,
          message: `Navigating to /cagnotte/${jackpotSlug}/`,
        };
      },
    },
    [router],
  ...
  useSafeFrontendTool(
    {
      ...
      description:
        "Open the jackpot creation/editing form. If a jackpotId is provided, opens in edit mode for that jackpot.",
      parameters: z.object({
        jackpotId: z
          .string()
          .optional()
          .describe("Optional jackpot ID to edit an existing jackpot"),
      }),
      handler: async ({ jackpotId }) => {
        if (jackpotId) {
          const form = forms.find((f) => f.id === jackpotId);
          if (!form)
            return {
              success: false,
              error: `Jackpot "${jackpotId}" not found`,
            };
        ...
        return {
          success: true,
          message: jackpotId
            ? `Opening edit form for jackpot "${jackpotId}"`
            : "Opening new jackpot form",
        };
      },
    },
    [forms],
  ...
  const loadContributionStats = async (formId: string) => {
    try {
      const { data: contributions } =
        await client.models.JackpotContribution.list({
          filter: { jackpotFormId: { eq: formId } },
        });

      if (contributions) {
        const stats = calculateJackpotStats(
          contributions as JackpotContributionData[],
        );
        setFormStats((prev) => ({
          ...prev,
          [formId]: {
  ...
  useEffect(() => {
    ...
    const { unsubscribe } = client.models.StripeConnectAccount.observeQuery({
      ...
      next: ({ items }) => {
        setConnectAccount(
          (items[0] as Schema["StripeConnectAccount"]["type"]) || null,
        );
        setConnectLoading(false);
      },
      error: (error) => {
  ...
  if (!mounted) {
    return (
      <Container>
        <Section>
          <Title level={TitleLevel.LEVEL1}>Gestion des Cagnottes</Title>
  ...
  const handleDeleteForm = async (id: string, title: string) => {
    const confirmed = window.confirm(
      `Êtes-vous sûr de vouloir supprimer cette cagnotte : "${title}" ?`,
    );
    if (!confirmed) return;
    ...
    <>
      ...
      <Container>
        <Section>
          ...
          {isAuthenticated &&
            !connectLoading &&
            (!connectAccount || connectAccount.accountStatus !== "ACTIVE") && (
              <InfoBlock>
                <InfoBlockHeader
          ...
          <Box>
            <div style={{ overflowX: "auto" }}>
              {loading ? (
                ...
                <>
                  <Title level={TitleLevel.LEVEL2}>Mes cagnottes</Title>
                          ...
                          >
                            ...
                            >
                              <TableHead>
                                <TableTr>
                                  <TableTh
                                    className={flexStyles.isHiddenMobile}
                                  >
                ...
                <Box>
                  ...
                  <Text>
                    Vous devez être connecté pour créer des cagnottes.
                  </Text>
                </Box>
              )}
            </div>
