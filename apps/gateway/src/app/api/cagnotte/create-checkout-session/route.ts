...
export async function POST(request: NextRequest) {
  try {
    ...
    if (jackpotForm.status !== 'ACTIVE') {
      return NextResponse.json(
        { error: 'This jackpot is not accepting contributions' },
        { status: 400 }
      );
    }
    ...
    if (new Date() > deadline) {
      return NextResponse.json(
        { error: 'This jackpot has reached its deadline' },
        { status: 400 }
      );
    }
    ...
    if (paymentMethodType === 'SEPA') {
      if (!jackpotForm.sepaPaymentsAllowed) {
        return NextResponse.json(
          { error: 'SEPA payments are not enabled for this jackpot' },
          { status: 400 }
        );
      }
      ...
      if (jackpotForm.sepaPaymentsCutoffAt) {
        ...
        if (new Date() > cutoffDate) {
          ...
          return NextResponse.json(
            { error: `SEPA payments must be made at least ${sepaDays} days before deadline` },
            { status: 400 }
          );
        }
      }
    ...
    if (!jackpotForm.stripeAccountId) {
      return NextResponse.json(
        { error: 'No Stripe Connect account configured for this jackpot' },
        { status: 400 }
      );
    }
    ...
    if (connectAccount.accountStatus !== 'ACTIVE' || !connectAccount.chargesEnabled) {
      return NextResponse.json(
        { error: 'Creator Stripe account is not active or cannot accept charges' },
        { status: 400 }
      );
    }
    ...
    const piParams = buildPaymentIntentParams(
      amountCentimes,
      jackpotForm.stripeAccountId,
      feeConfig
    );
